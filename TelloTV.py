...
# 
face_cascade = compv.CascadeClassifier('cascades/data/haarcascade_frontalface_alt2.xml')
recognizer = compv.face.LBPHFaceRecognizer_create()
...
class FrontEnd(object):
    ...
    def run(self):
        ...
        while not should_stop:
            ...
            frame = compv.cvtColor(frame_read.frame, compv.COLOR_BGR2RGB)
            frameRet = frame_read.frame

            vid = self.tello.get_video_capture()

            if args.save_session:
                compv.imwrite("{}/tellocap{}.jpg".format(ddir,imgCount),frameRet)
            ...
            gray  = compv.cvtColor(frameRet, compv.COLOR_BGR2GRAY)
            ...
            if self.send_rc_control and not OVERRIDE:
                for (x, y, w, h) in faces:
                    ...
                    compv.rectangle(frameRet, (x, y), (end_cord_x, end_cord_y), fbCol, fbStroke)

                    # Draw the target as a circle
                    compv.circle(frameRet, (targ_cord_x, targ_cord_y), 10, (0,255,0), 2)

                    # Draw the safety zone
                    compv.rectangle(frameRet, (targ_cord_x - szX, targ_cord_y - szY), (targ_cord_x + szX, targ_cord_y + szY), (0,255,0), fbStroke)

                    # Draw the estimated drone vector position in relation to face bounding box
                    compv.putText(frameRet,str(vDistance),(0,64),compv.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
            ...
            compv.circle(frameRet, (cWidth, cHeight), 10, (0,0,255), 2)
            ...
            compv.putText(frameRet,show,(32,664),compv.FONT_HERSHEY_SIMPLEX,1,dCol,2)

            # Display the resulting frame
            compv.imshow(f'Tello Tracking...',frameRet)

        # On exit, print the battery
        self.tello.get_battery()

        # When everything done, release the capture
        compv.destroyAllWindows()
